...
const submit = () => {
    if (hasPermission('supplier_edit')) {
...
<template>
    <Head title="گۆڕینی فرۆشیار - "/>
    <div class="container mx-auto mt-4">
        <div class="flex items-center justify-between p-4 border border-gray-200 rounded-md">
            <h1 class="text-sm lg:text-xl">گۆڕینی فرۆشیار / {{ props.supplier.name }}</h1>
            <Link :href="route('suppliers.index')" class="primary-btn" v-if="hasPermission('supplier_edit')">
                <span>گەڕانەوە</span>
        ...
        <form @submit.prevent="submit" class="border border-gray-200 rounded-md p-4 mt-6">
            ...
            <button type="submit" class="primary-btn " :disabled="form.processing" v-if="hasPermission('supplier_edit')">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
